...
import { Plus, MapPin, Phone, Mail, X, Save } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentUser.clients.map(client => (
                    <div key={client.id} className="bg-zinc-900/40 backdrop-blur-md p-6 rounded-2xl border border-white/5 hover:border-[#ccff00]/30 transition-colors shadow-lg group">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <span className="text-[10px] bg-white/10 px-2 py-1 rounded text-zinc-300">{client.paymentMethod}</span>
                        </div>
                        ...
                        <div className="space-y-2 text-sm text-zinc-400">
                            ...
                            {client.address && (
                                <div className="flex items-center gap-2">
                                    <MapPin className="w-4 h-4 text-zinc-600" /> {client.address}
                                </div>
                            )}
                            {!client.phone && !client.email && !client.address && (
                                <p className="italic text-zinc-600">Sin datos de contacto adicionales</p>
                            )}
                        </div>
                ...
                {currentUser.clients.length === 0 && (
                    <div className="col-span-full py-12 text-center text-zinc-500 bg-white/5 rounded-2xl border border-dashed border-white/10">
                        ...
                        <button onClick={handleOpenModal} className="mt-4 text-[#ccff00] text-sm hover:underline">
                            Agregar el primero
                        </button>
                    </div>
                )}
                        ...
                        <form onSubmit={handleSaveClient} className="space-y-4">
                            ...
                            <div className="pt-4 flex justify-end gap-3">
                                <button 
                                    ...
                                    className="px-4 py-2 rounded-xl text-zinc-400 hover:bg-white/5 transition-colors font-medium"
                                >
                                    Cancelar
                                </button>
                                <button 
                                    type="submit"
                                    className="bg-[#ccff00] text-black px-6 py-2 rounded-xl font-bold shadow-lg hover:scale-105 transition-transform flex items-center gap-2"
                                >
