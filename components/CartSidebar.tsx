...
import { CartItem, Brand, ContactInfo, User, PaymentConfig, ResellerOrder } from '../types';
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[480px] z-50 shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'} ${bgMain}`}>
        <div className={`px-6 py-5 border-b flex items-center justify-between ${borderCol}`}>
            <div className="flex items-center gap-3"><ShoppingBag className={`w-5 h-5 ${isDark ? 'text-white' : isBio ? 'text-blue-900' : 'text-emerald-800'}`} /><h2 className={`text-lg font-bold ${textMain}`}>{checkoutStep === 'cart' && "Tu Carrito"}{checkoutStep === 'login' && "Identificación"}{checkoutStep === 'payment' && "Finalizar Pedido"}</h2></div>
        ...
        {checkoutStep === 'cart' && (
            <>
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {cart.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center space-y-4"><div className={`w-16 h-16 rounded-full flex items-center justify-center ${isDark ? 'bg-white/5' : isBio ? 'bg-blue-50' : 'bg-emerald-50'}`}><ShoppingBag className={`w-8 h-8 ${textMuted}`} /></div><p className={`${textMuted}`}>Tu carrito está vacío</p></div>
                    ) : (
                        cart.map((item) => {
                            const finalPrice = item.price - (item.price * ((item.discount || 0) / 100));
                            return (
                                <div key={item.id} className="flex gap-4 group">
                                    <div className={`w-24 h-24 rounded-xl overflow-hidden flex-shrink-0 border ${isDark ? 'bg-black/40 border-white/5' : 'bg-gray-50 border-gray-100'}`}><img src={item.image} alt={item.name} className="w-full h-full object-cover" /></div>
                                    <div className="flex-1 flex flex-col justify-between py-1">
                                        <div><div className="flex justify-between items-start"><h3 className={`font-bold line-clamp-1 ${textMain}`}>{item.name}</h3><span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${item.brand === 'informa' ? 'bg-[#ccff00] text-black' : item.brand === 'iqual' ? 'bg-indigo-600 text-white' : item.brand === 'biofarma' ? 'bg-blue-600 text-white' : 'bg-emerald-100 text-emerald-800'}`}>{item.brand}</span></div><p className="text-xs text-gray-500 mt-1">{item.category}</p></div>
                                        <div className="flex items-center justify-between mt-2">
                                            <div className={`flex items-center border rounded-lg ${isDark ? 'border-white/10' : 'border-gray-200'}`}><button onClick={() => onUpdateQuantity(item.id, -1)} className={`w-8 h-8 flex items-center justify-center transition-colors ${isDark ? 'text-gray-400 hover:bg-white/10' : 'text-gray-600 hover:bg-gray-50'}`}>-</button><span className={`px-2 text-sm font-medium w-8 text-center ${textMain}`}>{item.quantity}</span><button onClick={() => onUpdateQuantity(item.id, 1)} className={`w-8 h-8 flex items-center justify-center transition-colors ${isDark ? 'text-gray-400 hover:bg-white/10' : 'text-gray-600 hover:bg-gray-50'}`}>+</button></div>
                                            <div className="flex items-center gap-3"><div className="text-right">{item.discount && item.discount > 0 && (<div className="flex items-center justify-end gap-1"><span className="text-xs text-gray-500 line-through">${(item.price * item.quantity).toLocaleString()}</span><span className="text-[10px] bg-red-500/10 text-red-500 px-1 rounded font-bold">-{item.discount}%</span></div>)}<span className={`font-bold ${item.discount ? 'text-red-500' : textMain}`}>${(finalPrice * item.quantity).toLocaleString()}</span></div><button onClick={() => onRemoveItem(item.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-500/10 rounded-full transition-colors"><Trash2 className="w-4 h-4" /></button></div>
                                        </div>
                                    </div>
                                </div>
                            );
                ...
                {cart.length > 0 && (
                    <div className={`border-t p-6 ${isDark ? 'border-white/10 bg-black/20' : 'border-gray-100 bg-gray-50'}`}>
                        <div className="flex items-center justify-between mb-4"><span className={textMuted}>Total a pagar</span><span className={`text-2xl font-black ${textMain}`}>${total.toLocaleString()}</span></div>
                        <button onClick={handleInitialCheckoutClick} className={`w-full py-4 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2 text-lg shadow-xl hover:scale-105 ${accentBg} ${accentTextBtn}`}>FINALIZAR COMPRA</button>
                    </div>
                )}
            </>
        )}
